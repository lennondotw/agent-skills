...
const validateWorkspace = async (workspacePath) => {
  ...
  if (packageJson.name !== expectedPackageName) {
    return {
      valid: false,
      reason: `Expected package name ${expectedPackageName}, received ${packageJson.name ?? 'none'}.`,
    }
  }
...
const ensureEmptyDestination = async (destination) => {
  ...
  if (!stat) return
  if (!stat.isDirectory()) throw new Error(`Destination exists and is not a directory: ${destination}`)
