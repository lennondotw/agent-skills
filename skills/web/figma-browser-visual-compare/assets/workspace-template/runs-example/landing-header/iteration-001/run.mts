...
const settings = {
  ...
  figma: {
    ...
    format: 'png',
    tokenEnv: 'TODAY_FIGMA_DEV_TOKEN',
  },
...
async function getFigmaHierarchies(token: string) {
  ...
  const response = await figmaFetch(`files/${settings.figma.fileKey}/nodes?${params.toString()}`, token)
  const payload = await response.json()
  ...
  for (const comparison of comparisons) {
    ...
    if (!root) {
      throw new Error(`Figma did not return node hierarchy for ${comparison.name} (${comparison.figmaNodeId})`)
    }
...
async function preparePage(page: Page, comparison: Comparison) {
  ...
  if (comparison.browserSelector === '[data-landing-header-state]') {
    await page.waitForFunction(
      (state) => document.querySelector('[data-landing-header-state]')?.getAttribute('data-landing-header-state') === state,
...
async function compareImages({ actual, comparison, expected }: { actual: PNG; comparison: Comparison; expected: PNG }) {
  const groupDir = path.join(iterationDir, comparison.groupName)
  await fs.mkdir(groupDir, { recursive: true })

  const expectedSource = comparison.expectedCropCss ? cropPng(expected, comparison.expectedCropCss) : expected
  const compositedExpected = compositeOverBackground(expectedSource, settings.compare.compositeBackground)
  ...
  if (expectedForComparison.width !== actualForComparison.width || expectedForComparison.height !== actualForComparison.height) {
    throw new Error(
...
console.log(JSON.stringify({ figmaMe: { email: me.email, handle: me.handle, id: me.id } }, null, 2))
...
try {
  ...
  await fs.writeFile(path.join(iterationDir, 'measurements.json'), `${JSON.stringify(measurements, null, 2)}\n`)
  await fs.writeFile(path.join(iterationDir, 'report.json'), `${JSON.stringify(report, null, 2)}\n`)
