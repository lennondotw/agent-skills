...
const configSchema = z.object({
  ...
  compare: z
    .object({
      compositeBackground: z.tuple([z.number().min(0).max(255), z.number().min(0).max(255), z.number().min(0).max(255)]).default([255, 255, 255]),
  ...
  figma: z.object({
    ...
    tokenEnv: z.string().default('TODAY_FIGMA_DEV_TOKEN'),
  }),
  ...
  pageSetup: z
    ...
    .default({ hideSelectors: [], pauseVideos: true, waitAfterScrollMs: 250, waitAfterStyleMs: 250 }),
...
async function measureFigmaStates({ config, token }) {
  ...
  const response = await figmaFetch(`files/${config.figma.fileKey}/nodes?${params.toString()}`, token)
  const payload = await response.json()
  ...
  for (const state of config.states) {
    ...
    const namedMeasurements = state.figmaMeasurements.map((item) => {
      const matches = flattened.filter((node) => node.name === item.nodeName || node.characters === item.nodeName)
...
async function compareState({ actual, config, outputDir, reference, state }) {
  if (reference.image.width !== actual.image.width || reference.image.height !== actual.image.height) {
    throw new Error(
  ...
  const actualComparison = compositeOverBackground(actual.image, config.compare.compositeBackground)
