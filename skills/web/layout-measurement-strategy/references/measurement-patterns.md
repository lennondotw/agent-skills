...
Use when a layout algorithm must place _other_ elements against this height before first paint (the TCK/badge case). Mount the probe **inside the real subtree** (inline hidden, or a React portal into a container under the same themed / container / shadow ancestor) so it inherits vars, fonts, `@container` context, and shadow stylesheets — **not** on `document.body`.
