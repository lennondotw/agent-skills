...
import { trustHeight, MIN_TRUSTWORTHY_HEIGHT } from "./trustHeight";
...
export function MeasureInPlace({ children }: { children: React.ReactNode }) {
  ...
  useLayoutEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      if (trusted != null) { prev = trusted; setHeight(trusted); }
    });
...
Use when a layout algorithm must place *other* elements against this height before first paint (the TCK/badge case). Mount the probe **inside the real subtree** (inline hidden, or a React portal into a container under the same themed / container / shadow ancestor) so it inherits vars, fonts, `@container` context, and shadow stylesheets — **not** on `document.body`.
...
For an expand/collapse (`0 ↔ content height`), the height the container transitions to must come from an element that is **not** the one being constrained. Measure an **inner wrapper left at `height: auto`** and observe *it* with a `ResizeObserver`; the outer container clips it with `overflow: hidden` and animates its own height to the measured value. Reading the outer container's height would just read back what you set. Two supporting details:
